...
1. **Live policy copy is wrong for 5.0.** On September 15 both `https://stillviewapp.com/privacy.html` and `https://stillviewapp.com/terms.html` returned 200. The folder, sharing, and GitHub-draft corrections from September 6 are live. The AI Insights section on both pages still says Vision identifies categories and counts faces, that the language model "never receives image pixels," and that the feature needs macOS 26. In 5.0 the image goes to the on-device Foundation Models model, and the app requires macOS 27. The corrected pages are committed in the stillviewapp.com repository (`8397f05`, effective date Sept. 15, 2026) and wait for `./deploy.sh`. After deploying, confirm the privacy URL in App Store Connect.
2. **Screenshots.** Done on September 15: `marketing/screenshots-5.0.0/` holds eight 2880×1800 frames (clean and captioned) plus website WebP pairs, captured from the 5.0.0 Debug build with the snapshot rig. The landing page swap is committed in the site repository (`98b5941`) and also waits for `./deploy.sh`.
