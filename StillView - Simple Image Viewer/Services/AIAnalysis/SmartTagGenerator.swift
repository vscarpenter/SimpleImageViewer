...
final class SmartTagGenerator {
    ...
    func generateSmartTags(
        ...
        tags.append(contentsOf: generateSeasonalTags(classifications: classifications, objects: objects, scenes: scenes))
    ...
    // MARK: - Seasonal Tags (Phase 1)

    private func generateSeasonalTags(
        classifications: [ClassificationResult],
        objects: [DetectedObject],
        scenes: [SceneClassification]
    ) -> [SmartTag] {
        ...
        // Collect all identifiers for matching
        let allTerms = classifications.map { $0.identifier.lowercased() } +
                       objects.map { $0.identifier.lowercased() } +
                       scenes.map { $0.identifier.lowercased() }

        // Helper to count matches against indicator set
        func countMatches(_ indicators: Set<String>) -> Int {
            allTerms.filter { term in
                indicators.contains(where: { term.contains($0) })
            }.count
        }

        // Check each season
        let winterMatches = countMatches(AIAnalysisConstants.winterIndicators)
        let summerMatches = countMatches(AIAnalysisConstants.summerIndicators)
        let autumnMatches = countMatches(AIAnalysisConstants.autumnIndicators)
        let springMatches = countMatches(AIAnalysisConstants.springIndicators)

        // Only tag if we have enough confidence (2+ matches or 1 strong match)
        let seasonScores = [
            ("Winter", winterMatches),
            ("Summer", summerMatches),
            ("Autumn", autumnMatches),
            ("Spring", springMatches)
        ]

        // Require 2+ keyword matches to avoid false positives from a single term
        if let bestMatch = seasonScores.max(by: { $0.1 < $1.1 }), bestMatch.1 >= 2 {
            // Confidence based on number of matches
            let confidence = min(0.9, 0.5 + Double(bestMatch.1) * 0.15)
    ...
    private func inferActivitiesFromObjects(_ objects: [DetectedObject]) -> [(String, Double)] {
        ...
        // Phase 1: Add studying/learning activity
        // Books + desk/study context = Studying
        let studyContext = ["desk", "study", "library", "notebook", "pencil", "pen", "highlighter", "notes"]
        if objectsContainAny(readingObjects) && objectsContainAny(studyContext) {
            activities.append(("Studying", 0.75))
        }

        // Phase 1: Add gardening activity
        let gardenObjects = ["plant", "flower pot", "watering can", "garden", "trowel", "shovel",
                            "pruning shears", "wheelbarrow", "rake", "hoe", "seeds", "soil"]
        if objectsContainAny(gardenObjects) {
            // If multiple garden objects, higher confidence
            let gardenCount = objectIds.filter { id in
                gardenObjects.contains(where: { id.contains($0) })
            }.count
            let confidence = gardenCount >= 2 ? 0.8 : 0.65
            activities.append(("Gardening", confidence))
        }

        // Phase 1: Add gaming activity
        let gamingObjects = ["controller", "gamepad", "joystick", "console", "playstation", "xbox",
                            "nintendo", "gaming", "video game", "headset"]
        if objectsContainAny(gamingObjects) {
            activities.append(("Gaming", 0.75))
        }

        // Phase 1: Add crafting/DIY activity
        let craftObjects = ["scissors", "glue", "yarn", "needle", "thread", "fabric", "sewing",
                           "knitting", "crochet", "craft", "paint brush", "canvas", "easel",
                           "toolbox", "hammer", "screwdriver", "drill", "saw", "workbench"]
        if objectsContainAny(craftObjects) {
            // Check if more art-focused or tool-focused
            let artObjects = ["paint brush", "canvas", "easel", "yarn", "needle", "fabric"]
            let toolObjects = ["hammer", "screwdriver", "drill", "saw", "workbench"]
            let hasArt = objectIds.contains { id in artObjects.contains(where: { id.contains($0) }) }
            let hasTools = objectIds.contains { id in toolObjects.contains(where: { id.contains($0) }) }

            if hasArt {
                activities.append(("Crafting", 0.75))
            } else if hasTools {
                activities.append(("DIY", 0.75))
            } else {
                activities.append(("Creating", 0.7))
            }
        }

        // Phase 1: Add drinking/beverages activity
        let drinkObjects = ["wine glass", "beer glass", "cocktail", "coffee cup", "tea cup",
                           "mug", "bottle", "champagne", "drink"]
        if objectsContainAny(drinkObjects) {
            activities.append(("Drinking", 0.65))
        }
    ...
    private func generateUseCaseTags(
        ...
        case .landscape:
            ...
            // Wallpaper requires high resolution AND at least medium quality
            if isHighResolution && (isHighQuality || quality == .medium) {
                tags.append(SmartTag(name: "Wallpaper", category: .useCase, confidence: 0.8, isAutoGenerated: true))
            }
            // Print requires good resolution and quality
            if megapixels >= AIAnalysisConstants.printQualityMinMegapixels && isHighQuality {
                tags.append(SmartTag(name: "Print", category: .useCase, confidence: 0.7, isAutoGenerated: true))
            }
