...
final class QualityAssessmentService {
    ...
    func assessQuality(
        ...
        // Run sharpness and exposure analysis in parallel
    ...
    func detectSharpness(_ cgImage: CGImage) async throws -> Double {
        return try await withCheckedThrowingContinuation { continuation in
            analysisQueue.async {
                ...
                let normalizedSharpness = min(1.0, variance / 255.0)
    ...
    func calculateOverallQuality(_ metrics: ImageQualityAssessment.Metrics, imageSize: CGSize) -> ImageQuality {
        ...
        // Exposure component - penalize over/under exposure
        let exposureQuality = 1.0 - abs(metrics.exposure - AIAnalysisConstants.optimalExposure) * 2.0
        qualityScore += max(0, exposureQuality) * AIAnalysisConstants.qualityExposureWeight
